...
add_action( 'init', function() {
           $type = 'vlog';
           $labels = xcompile_post_type_labels('Vlog', 'Vlogs');
           
           $arguments = [
/*        'register_meta_box_cb' => 'vlog_meta_box', */
           'rewrite' => array( 'slug' => 'vlog','with_front' => false ),
           'has_archive' => true,
           'rest_base' => 'vlogs',
           'show_in_rest' => true,
           'hierarchical' => false,
           ...
           'public' => true,
           'description' => 'Administrare Pagina Vlog',
...
register_meta('post', 'vlog_link', [
              'object_subtype' => 'vlog', // Limit to a post type.
              'type'           => 'string',
              'description'    => 'link',
              'single'         => true,
              'show_in_rest'   => true,
...
function register_rest_images(){
    register_rest_field( array('vlog'),
                        'fimg_url',
                        array(
...
/* Adding posts rest images */

add_action('rest_api_init', 'register_rest_post_images' );

function register_rest_post_images(){
    register_rest_field( array('post'),
                        'imagine_postare',
                        array(
                              'get_callback'    => 'get_rest_post_featured_image',
                              'update_callback' => null,
                              'schema'          => null,
                              )
                        );
}
function get_rest_post_featured_image( $object, $field_name, $request ) {
    if( $object['featured_media'] ){
        $img = wp_get_attachment_image_src( $object['featured_media'], 'app-thumb' );
        return $img[0];
    }
...
add_action( 'init', function() {
           ...
           $arguments = [
/*        'register_meta_box_cb' => 'vlog_meta_box', */
           'rewrite' => array( 'slug' => 'cars-article','with_front' => false ),
           'has_archive' => true,
           'rest_base' => 'articles',
           'show_in_rest' => true,
           'hierarchical' => false,
           'supports' => ['title', 'editor', 'revisions', 'page-attributes', 'thumbnail', 'custom-fields', 'excerpt'],
           'public' => true,
           'description' => 'Administrare Pagina Tips&Cars',
...
add_action('rest_api_init', 'register_rest_cars_images' );

function register_rest_cars_images(){
    register_rest_field( array('article'),
                        'f_img_url',
                        array(
                              'get_callback'    => 'get_rest_cars_featured_image',
                              'update_callback' => null,
                              'schema'          => null,
                              )
                        );
}
function get_rest_cars_featured_image( $object, $field_name, $request ) {
    if( $object['featured_media'] ){
        $img = wp_get_attachment_image_src( $object['featured_media'], 'app-thumb' );
        return $img[0];
    }
